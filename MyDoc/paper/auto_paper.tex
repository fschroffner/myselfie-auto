...
The feature is intentionally minimal, leaving several local extensions open, none of them architectural. (i)~\emph{More to deduce:} \texttt{auto} yields one of only two types because C* has only two; adding a type to Selfie would immediately give it more to deduce, with no change to the deduction mechanism. (ii)~\emph{Targeted diagnostics:} forward and circular \texttt{auto} are reported through the generic undeclared-identifier error, where a dedicated message would help. (iii)~\emph{Wider scope:} extending \texttt{auto} to procedure return types or globals is conceivable, though globals are allocated before any statement is parsed and would need a different allocation strategy than the lazy stack push used here.
